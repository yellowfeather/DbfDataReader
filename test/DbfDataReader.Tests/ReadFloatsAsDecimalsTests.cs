...
[Collection("dbase_31")]
public class ReadFloatsAsDecimalsTests : DbaseTests
{
    private const string Dbase31FixturePath = "../../../../fixtures/dbase_31.dbf";

    private static readonly DbfDataReaderOptions Options = new()
    {
        ReadFloatsAsDecimals = true
    };

    public ReadFloatsAsDecimalsTests() : base(Dbase31FixturePath, Options)
    {
    }
    ...
    [Fact]
    public void Should_read_floats_as_decimals()
    {
        var dbfRecord = new DbfRecord(DbfTable);
        DbfTable.Read(dbfRecord);
        var dbfValue = dbfRecord.Values[10];
        dbfValue.GetValue().ShouldBeOfType<decimal>();
    }
    
    [Fact]
    public void Should_not_read_floats_as_decimals()
    {
        DbfTable.ReadFloatsAsDecimals = false;
